...
def generate_frontier(cfg, llm: LLMClient, knowledge_ctx: list[dict],
                      recent_tasks: list[dict], projects: list[dict] | None = None) -> dict:
    system = (
        "# ROLE: frontier\n\n"
        ...
        "根据用户的 Knowledge State（已掌握的概念及其掌握程度）、近期研究历史和**进行中的项目**，"
        "推荐 3 个本周值得深入研究的话题。\n"
        "要求：\n"
        "- 话题应切中用户的知识缺口（partial / unknown 的概念优先），避免推荐用户已经完全掌握的主题。\n"
        "- 尽量让话题与用户的进行中项目相关（结合项目技术栈/描述），并指出相关项目。\n"
        "- 每个话题包含：topic（一行标题）、why（一段话说明为什么本周值得关注）、"
        "priority（high/medium/low）、related_projects（相关的项目名数组，无关则为空数组）。\n"
        "- 只输出 JSON，格式：{\"suggestions\": [{\"topic\": \"...\", \"why\": \"...\", "
        "\"priority\": \"high|medium|low\", \"related_projects\": [\"项目名\"]}]}\n"
    ...
    if recent_tasks:
        ...
        ctx_lines.append("（无）")
    ctx_lines.append("")
    ctx_lines.append("## 用户进行中的项目")
    if projects:
        for p in projects:
            ctx_lines.append(f"- {p['name']}（描述:{p.get('description','')[:80]} 技术栈:{','.join(p.get('tech_stack',[]))}）")
    else:
        ctx_lines.append("（无）")
    user = "\n".join(ctx_lines)
    ...
    return {
        "suggestions": [
            {"topic": s.get("topic", ""), "why": s.get("why", ""),
             ...
             "related_projects": s.get("related_projects") or []}
