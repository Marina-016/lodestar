...
左侧保留核心栏目：研究对话、知识库、研究历史、实验项目和我的项目，不再放任务列表。

1. 录制从中间输入框开始，可直接使用脚本中的预设问题，点击「发送」或按 Enter 后自动回放。
2. 每次研究结束后，当前研究卡底部只保留一个统一的「下一步建议」按钮，不同时显示多个分支。
3. 如有待确认记忆，按钮会定位到 Knowledge State 确认区；其他情况则引导用户查看关联代码。
4. 新的研究输出会替换旧的建议控制，保持中间区域始终只有一个下一步入口。
