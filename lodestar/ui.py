...
def _demo_task_catalog() -> list[dict]:
    """Return stable, recording-friendly task entry points for the UI."""
    from lodestar.demo import DEMO_FRONTIER

    return [
        {"id": "demo-frontier-weekly", "title": "\u672c\u5468 Agent \u524d\u6cbf\u7814\u7a76", "description": "\u626b\u63cf\u672c\u5468 Agent \u8bba\u6587\uff0c\u89e3\u91ca\u6982\u5ff5\u5e76\u6309 Lodestar \u9879\u76ee\u7f3a\u53e3\u6392\u5e8f\u3002", "prompt": DEMO_FRONTIER["goal"], "tags": ["Research", "Trace"], "status": "\u53ef\u6f14\u793a", "mode": "Curated Demo Replay"},
        {"id": "demo-ls-002", "title": "Memory Trust Gate", "description": "\u5224\u65ad\u4e00\u6761\u8bb0\u5fc6\u662f\u5426\u5e94\u8be5\u8fdb\u5165 Knowledge State\uff0c\u5e76\u68c0\u67e5\u6765\u6e90\u4e0e\u51b2\u7a81\u98ce\u9669\u3002", "prompt": "\u5e2e\u6211\u5224\u65ad\u8fd9\u6761\u8bb0\u5fc6\u662f\u5426\u5e94\u8be5\u8fdb\u5165 Knowledge State\u3002", "tags": ["Memory", "Research", "Trace"], "status": "\u53ef\u6f14\u793a", "mode": "Curated Demo Replay"},
        {"id": "demo-ls-003", "title": "Skill \u8de8\u4efb\u52a1\u8fc1\u79fb", "description": "\u7814\u7a76\u5b50\u4efb\u52a1\u7ea7 Skill \u5982\u4f55\u8fc1\u79fb\uff0c\u5e76\u628a\u6210\u529f\u8f68\u8ff9\u8f6c\u6210\u53ef\u8bc4\u4f30\u5019\u9009\u3002", "prompt": "\u8bf7\u7814\u7a76 Skill \u5982\u4f55\u8de8\u4efb\u52a1\u8fc1\u79fb\uff0c\u5e76\u5224\u65ad Lodestar \u5e94\u8be5\u9a8c\u8bc1\u4ec0\u4e48\u3002", "tags": ["Skill", "Eval", "Trace"], "status": "\u53ef\u6f14\u793a", "mode": "Curated Demo Replay"},
        {"id": "demo-ls-001", "title": "Tool-use Eval", "description": "\u628a\u5de5\u5177\u8c03\u7528\u8f68\u8ff9\u53d8\u6210\u53ef\u5ba1\u8ba1\u7684\u56de\u5f52\u8bc4\u6d4b\u96c6\u3002", "prompt": "\u8bf7\u7814\u7a76 Tool-use Eval\uff0c\u5224\u65ad Lodestar \u5982\u4f55\u628a\u5de5\u5177\u8c03\u7528\u8f68\u8ff9\u53d8\u6210\u56de\u5f52\u96c6\u3002", "tags": ["Tool", "Eval", "Trace"], "status": "\u53ef\u6f14\u793a", "mode": "Curated Demo Replay"},
        {"id": "demo-ls-004", "title": "Context Budget", "description": "\u5728 Token \u9884\u7b97\u5185\u9009\u62e9\u66f4\u6709\u4ef7\u503c\u3001\u5197\u4f59\u66f4\u4f4e\u7684 Skill \u7ec4\u5408\u3002", "prompt": "\u8bf7\u7814\u7a76 Context Budget\uff0c\u5224\u65ad Lodestar \u5e94\u8be5\u5982\u4f55\u9009\u62e9 Skill\u3002", "tags": ["Context", "Skill", "Eval"], "status": "\u53ef\u6f14\u793a", "mode": "Curated Demo Replay"},
    ]
...
(function(){
  ...
  function animateBrief(el,text){
    return new Promise(resolve=>{let i=0;const interval=38;const duration=Math.min(14000,Math.max(4800,text.length*18));const step=Math.max(1,Math.ceil(text.length/(duration/interval)));const timer=setInterval(()=>{i=Math.min(text.length,i+step);el.innerHTML=md(text.slice(0,i));scrollChat();if(i>=text.length){clearInterval(timer);scrollChat(true);resolve();}},interval);});
  }
...
"""
    task_extension = r"""
<style>
.welcome .suggestions{display:none!important}.composer-foot{margin-top:3px;position:relative;top:-2px}
.workspace{max-width:1320px;padding:14px 28px 28px}.topbar{margin-bottom:10px}.chat-frame{grid-template-columns:minmax(0,1fr) 210px;gap:12px}.chat-header{padding:12px 16px}.context-card{padding:13px}.chat-scroll{padding:14px 18px 10px}.composer{padding:10px 12px 12px}
.next-actions{margin-top:15px;padding-top:13px;border-top:1px solid var(--line);display:flex;justify-content:flex-end}.next-primary{display:inline-flex;align-items:center;gap:7px;padding:8px 11px;border:1px solid color-mix(in srgb,var(--acc) 55%,var(--line));border-radius:9px;background:color-mix(in srgb,var(--acc) 10%,var(--panel));color:var(--acc);font-size:11px;font-weight:800;text-align:left}.next-primary:after{content:'\2197';font-size:13px}.next-primary:hover{background:var(--acc);color:var(--on-acc);border-color:var(--acc)}
html,body{height:100%}body.chat-mode{overflow:hidden}body.chat-mode .app-shell{height:100vh;min-height:100vh;overflow:hidden}body.chat-mode .workspace{height:100vh;min-height:0;overflow:hidden}body.chat-mode #view-chat{height:calc(100vh - 94px);min-height:0}body.chat-mode .chat-frame{height:100%;min-height:0}body.chat-mode .chat-panel{height:100%;display:flex;flex-direction:column;min-height:0}body.chat-mode .chat-scroll{flex:1;min-height:0;max-height:none;overflow-y:auto}
@media(max-width:900px){.workspace{padding:18px 22px 28px}.chat-frame{grid-template-columns:1fr}.context-card{padding:13px}}
@media(max-width:600px){.workspace{padding:14px 14px 20px}body.chat-mode .workspace{height:calc(100vh - 54px);overflow:hidden}body.chat-mode #view-chat{height:100%}.next-actions{display:block}.next-primary{width:100%;justify-content:space-between}}
</style>
<script>
(function(){
  ...
  const form=document.querySelector('#composer'),baseSubmit=form&&form.onsubmit;
...
"""
    ...
    return base + extension_body + streaming_extension + task_extension + "</body></html>"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/demo/tasks":
            cfg = load_config()
            return self._send(200, {"mode": "curated_replay" if cfg.demo_replay else "live", "tasks": _demo_task_catalog()})
        if path == "/api/health":
            cfg = load_config()
