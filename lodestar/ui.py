...
def _run_codex_conversation(task_id: str, goal: str, cfg) -> str | None:
    """Run the opt-in Codex Harness path; return None so the caller can fall back."""
    try:
    ...
    except Exception:  # noqa: BLE001 - harness failure must not break the UI
        return None
...
def _run_research(task_id: str, goal: str, cfg, conversation_id: str | None = None) -> None:
    ...
    try:
        if getattr(cfg, "conversation_harness", "loop") == "codex":
            answer = _run_codex_conversation(task_id, goal, cfg)
            if answer:
                repo.finish_task(ws.conn, task_id, answer, "finished",
                                 metrics={"conversation_harness": "codex", "mcp": True})
